...
# ════════════════════════════════════════════════════════════════════
# WAG ULTIMATE: Compresión Semántica + Visualización Interactiva
# Combina: Doc4 (156x compression) + Doc3 (ti.GUI raymarching)
# ════════════════════════════════════════════════════════════════════
...
ALPHA = 0.5           # Dispersión
G = -1.0              # Nonlinearidad (focusing)
...
# ────────────────────────────────────────────────────────────────────
# INICIALIZACIÓN: Red Cósmica (Doc 4)
# ────────────────────────────────────────────────────────────────────
...
def initialize_cosmic_web(num_galaxies: ti.i32, scale: ti.f32):
    ...
    OPTIMIZADO: Paraleliza por píxeles en lugar de galaxias para evitar bloqueos.
    """
    # Iteramos sobre cada vóxel del espacio en paralelo (GPU friendly)
    for i, j, k in ti.ndrange(RESOLUTION, RESOLUTION, RESOLUTION):
        total_real = 0.0
        total_imag = 0.0
        
        # Cada vóxel comprueba su contribución de todas las galaxias localmente
        for gid in range(num_galaxies):
            ...
            # Cálculo de distancia toroidal (opcional, aquí simple)
            dx = ti.f32(i) - cx
            dy = ti.f32(j) - cy
            dz = ti.f32(k) - cz
            r_sq = dx*dx + dy*dy + dz*dz
            
            # Cutoff optimization: Solo calculamos si estamos cerca de la galaxia
            if r_sq < (width * 5.0)**2:
        ...
        # Escribimos el resultado acumulado
        psi_real[i, j, k] = total_real
        psi_imag[i, j, k] = total_imag

# ────────────────────────────────────────────────────────────────────
# FÍSICA: Split-Step (Conserva Norma)
# ────────────────────────────────────────────────────────────────────
...
@ti.kernel
def step2(dt: ti.f32, g: ti.f32):
    for i, j, k in ti.ndrange(RESOLUTION, RESOLUTION, RESOLUTION):
        r, im = psi_real_new[i,j,k], psi_imag_new[i,j,k]
        rho = r*r + im*im
        theta = g * rho * dt
        c, s = ti.cos(theta), ti.sin(theta)
...
# ────────────────────────────────────────────────────────────────────
# RAYMARCHING: Decodificador Holográfico (Doc 3)
# ────────────────────────────────────────────────────────────────────
...
def sample_phase(pos: ti.math.vec3) -> ti.f32:
    """Sampleo de fase para coloración (Corregido: usando max/min en lugar de clamp)"""
        ...
        for _ in range(num_steps):
            ...
            if (0 <= pos.x < RESOLUTION and 
                ...
                if rho > 0.005:
                    ...
                    # Color basado en fase (rueda de color)
                    ph = sample_phase(pos)
                    heat = ti.min(rho * 8.0, 1.0)
                    
                    # RGB de fase
                    r_val = 0.5 + 0.5 * ti.cos(ph)
                    g_val = 0.5 + 0.5 * ti.cos(ph + 2.094)  # 2π/3
                    b_val = 0.5 + 0.5 * ti.cos(ph + 4.189)  # 4π/3
...
# ────────────────────────────────────────────────────────────────────
# ANÁLISIS: Compresión Semántica (Doc 4)
# ────────────────────────────────────────────────────────────────────
...
def main():
    print("\n" + "="*60)
    print("WAG ULTIMATE: Compresión Semántica + Visualización 3D")
    print("="*60)
    print("Controles:")
    ...
    # GUI
    gui = ti.GUI("WAG Ultimate - Universe Simulator", 
    ...
    while gui.running:
        ...
        gui.text(f"WAG UNIVERSE - {num_galaxies} Galaxies", 
        ...
        gui.text(f"Sparsity: {sparsity:.1f}% empty", 
